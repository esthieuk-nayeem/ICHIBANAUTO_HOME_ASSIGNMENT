...
class Booking(models.Model):
    ...
    mechanic = models.ForeignKey(User, on_delete=models.CASCADE, related_name='booking_mechanic')
    status = models.CharField(max_length=50, choices=statuses)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.user.full_name}'s booking for {self.car.registration_plate} on {self.appointment_date} at {self.appointment_time}"
